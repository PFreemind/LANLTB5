import os
import time
import readZoomVideo as z
import cv2
import glob
import datetime
import numpy as np
import pytesseract
import matplotlib.pyplot as plot

def getDigit( input_image, x1, x2, y1, y2):
    # Load the 7x5 character templates for digits 0-9
    template = cv2.imread(f'digit_templates/7by5-digits.jpeg', cv2.IMREAD_GRAYSCALE)
    cv2.imshow('test', template )
    templates = []
    h, w  = template.shape
    _, binary = cv2.threshold(template, 128, 255, cv2.THRESH_BINARY)

# Invert the binary image
    inverted_binary = binary#cv2.bitwise_not(binary)
    step = float(w-17)/10.
    intercept = 5

    for i in range (10):
        
     #   print( int( (w*i) /10.),"  ", int( ((i+1) *w) /10.))
        templates.append( inverted_binary[  15:h-15,  max(0, int( (step*float(i) + intercept)))  : int( (step*float(i+1) + intercept) ) ] ) #int( ((w+1) *i) /10.) ] )# int( (w*i) /10.) : int( ((w+1)*i )/10) ]  )
    #    print(templates[i])
        cv2.imshow('digit'+str(i), templates[i])
    # Load your input image

    # Initialize a list to store the matching results
    matches = []
    #scale input image to match templates
    cropped_image = input_image[y1:y2, x1:x2]
    scaled_image = cv2.resize(cropped_image, (templates[0].shape[1], templates[0].shape[0]))
    # Loop through the templates and perform convolution
    for template in templates:
        result = cv2.matchTemplate(scaled_image, template, cv2.TM_CCOEFF_NORMED)
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
        matches.append(max_val)
        print(max_val)

    # Find the index of the best matching template
    best_match_index = np.argmax(matches)

    # You can set a threshold to determine if a match is valid
    threshold = 0.1
 #   print(matches[best_match_index])
    if matches[best_match_index] >= threshold:
        recognized_digit = best_match_index
      #  print ("Look at me, I did it!")
    else:
        recognized_digit = None

    # Print the recognized digit (or None if no valid match)
 #   print("Recognized Digit:", recognized_digit)
    
    return recognized_digit


def getReading(imgFile, x1 = 1261, x2=1312, y1=1412, y2 =1439):
#with a different angle and instrument, this will have to be rewritten.
# open the jpeg
    im = cv2.imread(str(imgFile), cv2.IMREAD_GRAYSCALE)
    cv2.imshow('input image', im)#
    roi = im[y1:y2, x1:x2] #crop
    cv2.imshow('cropped', roi)#
    rot =  cv2.rotate(roi, cv2.ROTATE_180)#roate 180 deg
    angle = -6
# Calculate the rotation matrix
    height, width = rot.shape[:2]
    rotation_matrix = cv2.getRotationMatrix2D((width / 2, height / 2), angle, 1)
# Apply the rotation to the image
    rotated_image = cv2.warpAffine(rot, rotation_matrix, (width, height))
 #   rect = np.array([[24,230], [1253,50], [1227,449], [11,569] ])
    warped = rotated_image# z.four_point_transform(gray_roi, rect)# correct for skew
    unsharp_image = cv2.addWeighted(warped, 2.0, gaussian_3, -1.0, 0)
    cv2.imshow('sharpened', unsharp_image)
    binary = cv2.adaptiveThreshold( warped,255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 101, -7) # adaptive thresholding
    cv2.imshow('binary', binary)#
        
    # parsing into individual chars
#return currenReading
    
    return reading



######## main functionality ####################

#run over a directory of images

today = str(datetime.date.today())

stamp = time.time()
filename = "currentReadings"+today+".csv"
file = open(filename, 'w')

currents = []
times = []
dir ="/Users/patfreeman/Desktop/Pi_captures/Pi_captures/*jpg"
pics =glob.glob(dir)
x1 = 1261 -1
x2= 1350 - 3
y1= 1057 - 10
y2 = 1089 +10
for pic in pics:
    stamp = pic.split("/")[-1].split("_")[1].split(".")[0]
    #print(stamp)
    im = cv2.imread(pic, cv2.IMREAD_GRAYSCALE)
    roi = im[y1:y2, x1:x2] #crop
    cv2.imshow('cropped', roi)#
    rot =  cv2.rotate(roi, cv2.ROTATE_180)#roate 180 deg
  #  rect = np.array([[24,230], [1253,50], [1227,449], [11,569] ])
    angle = -2
# Calculate the rotation matrix
    height, width = rot.shape[:2]
    rotation_matrix = cv2.getRotationMatrix2D((width / 2, height / 2), angle, 1)
# Apply the rotation to the imageangle = 40
# Calculate the rotation matrix
    rotated_image = cv2.warpAffine(rot, rotation_matrix, (width, height))

# Apply the rotation to the image
    warped = rotated_image#z.four_point_transform(rot, rect)# correct for skew
    gaussian_3 = cv2.GaussianBlur(warped, (7, 7), 4.0)
    unsharp_image = warped#cv2.addWeighted(warped, 2.0, gaussian_3, -1.0, 0)
    cv2.imshow('sharpened', unsharp_image)
    
 #   rect = np.array([[24,230], [1253,50], [1227,449], [11,569] ])
    binary = cv2.adaptiveThreshold( unsharp_image,255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 7, -10) # adaptive thresholding
    
    chars = []
    ytrim = 10
    h, w = binary.shape

    chars.append(binary [0 +  ytrim: h-  ytrim, 0:int(w/6) ]  )
    chars.append(binary [0 +  ytrim: h-  ytrim, int(w/6) : 2*int(w/6) ]  )
    chars.append(binary [0 +  ytrim: h-  ytrim,2*int(w/6) : 3*int(w/6) ]  )
    chars.append(binary [0 +  ytrim: h-  ytrim, 3*int(w/6) : int(4* w/6) ]  )
    chars.append(binary [0 +  ytrim: h-  ytrim, 4*int(w/6) : 5*int(w/6) ]  )
    chars.append(binary [0 +  ytrim: h-  ytrim, 5*int(w/6) : w ]  )

    reading =""
    noneFlag = 0
    i=0
    config = ' -c tessedit_char_whitelist=0123456789 --psm 7 --psm 10'
    extracted_text = pytesseract.image_to_string(binary, config=config)
    for char in chars:
        #extracted_text = pytesseract.image_to_string(char, config=config)
        xx1=0
        yy1=0
        xx2 = char.shape[1]
        yy2 = char.shape[0]
        extracted_text = pytesseract.image_to_string(char, config=config)
        if i < 6:
            reading+=str(extracted_text)
        if i == 0:
            reading+="."
   #     print("fuck")
        cv2.imshow('char'+str(i), char )
    #    print("fuck")
        if extracted_text is None:
            noneFlag = 1
            continue
        i+=1

    print("reading: "+str(reading))
    config = ' -c tessedit_char_whitelist=0123456789 --psm 7 '
    extracted_text = pytesseract.image_to_string(binary, config=config)
    cv2.putText(im, str(extracted_text), (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0))
    cv2.imshow('binary', binary)#
    cv2.imshow('input image', im)#
    times.append(times)
  #  current = getReading(pic)
    current = 0#int(extracted_text)
    print(current)
    file.write(str(stamp)+", " +  str(current) +"\n")
    currents.append(current)
    if cv2.waitKey(100) & 0xFF == ord('q'):
        break
            
    #chars
    
file.close()

#plot.plot(times, currents)

#take images and process them 1-by-1, fill a .csv with timestamps and current readings
'''
with  open(file, 'w') as f:
    stamp = time.time()
    
    pic = "image_"+str(stamp)+".jpg"
    cmd = "raspistill -n -t 1000 - tl 1000 -dt -o " + pic
    os.system(cmd)
    f.write(stamp)
    f.write(",")
    current = getReading(pic)
    f.write(str(stamp)+","+str(current)+"\n")
'''



